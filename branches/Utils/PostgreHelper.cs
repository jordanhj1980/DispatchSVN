...
{
    public class PostgreHelper : DBHelper
    {
        /// <summary>
        /// 得到数据条数
        /// </summary>
        public int GetCount(DbConnection conn, string tblName, string condition)
        {
            StringBuilder sql = new StringBuilder("select count(*) from " + tblName);
            if (!string.IsNullOrEmpty(condition))
                sql.Append(" where " + condition);

            object count = ExecuteScalar(conn, CommandType.Text, sql.ToString(), null);
            return int.Parse(count.ToString());
        }

        /// <summary>
        /// 执行查询，返回DataSet
        /// 用NpgsqlDataAdapter.Fill方法，把检索的结果集放到DataSet object中
        /// </summary>
        public DataSet ExecuteQuery(DbConnection conn, CommandType cmdType, string cmdText,
            params DbParameter[] cmdParms)
        {
            using (NpgsqlCommand cmd = new NpgsqlCommand())
            {
                ...
                using (NpgsqlDataAdapter da = new NpgsqlDataAdapter(cmd))
                {
                    ...
                    // Fill function opens and closes the database connection for you
                    da.Fill(ds, "ds");
        ...
        /// 执行查询，返回DataReader
        /// </summary>
        public DbDataReader ExecuteReader(DbConnection conn, CommandType cmdType, string cmdText,
            params DbParameter[] cmdParms)
        {
            using (NpgsqlCommand cmd = new NpgsqlCommand())
            {
